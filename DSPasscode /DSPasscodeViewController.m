...
@synthesize manager,formatingDict,passcodeLabel,passcodeTextfield,backgroundImageView,delegate,ispasscodeCheck;
...
- (void)viewDidLoad
{
    ...
    {
        self.backgroundImageView.image=[formatingDict objectForKey:@"image"];
    }
...
-(void)matchPasscode
{
    ...
    else
    {
        ...
        if([delegate respondsToSelector:@selector(confirmPasscodeMismatch)])
        {
            [delegate confirmPasscodeMismatch];
        }
    }
}
...
-(IBAction)numberKeyPressed:(UIButton *)sender
{
    ...
    if( length1<4)
    {
        ...
        if(sender.tag<10)
        {
            passStr=[self.passcodeTextfield.text stringByAppendingFormat:@"%i",sender.tag];
            self.passcodeTextfield.text=passStr;
            [self textFieldValueChanged:self.passcodeTextfield];
        }
        else if(sender.tag==10){
            passStr=[self.passcodeTextfield.text stringByAppendingFormat:@"%i",0];
            self.passcodeTextfield.text=passStr;
            [self textFieldValueChanged:self.passcodeTextfield];
        }
